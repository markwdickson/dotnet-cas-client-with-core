...
namespace DotNetCoreCas.Utils
{
    ...
    {
        public static Uri rawUrl(this Microsoft.AspNetCore.Http.HttpRequest request, ICasOptions options)
        {
            ...
            var host = request.Host.Value.Replace("localhost", options.LocalHost);
            return new Uri(request.Scheme + "://" + host + path + request.QueryString.ToUriComponent());
        }
    }
}